...
func (_q *ContactQuery) QueryAddresses() *AddressQuery {
...
func (_q *ContactQuery) WithAddresses(opts ...func(*AddressQuery)) *ContactQuery {
	query := (&AddressClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if query := _q.withAddresses; query != nil {
		if err := _q.loadAddresses(ctx, query, nodes,
			func(n *Contact) { n.Edges.Addresses = []*Address{} },
			func(n *Contact, e *Address) { n.Edges.Addresses = append(n.Edges.Addresses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ContactQuery) loadAddresses(ctx context.Context, query *AddressQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *Address)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(address.FieldContactID)
	}
	query.Where(predicate.Address(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(contact.AddressesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
